"""Resolve a quote's current price and prior regular-session close."""
...
def _market_date(value: Any, market_tz: ZoneInfo = US_EASTERN):
    if not value:
        return None
    try:
...
def resolve_day_quote(
    quote: dict[str, Any], candles: Iterable[dict[str, Any]], market_tz: ZoneInfo = US_EASTERN
) -> DayQuote:
    ...
    market_tz is the exchange's own timezone (default US Eastern; pass KOREA
    for KRX). The quote's session date must be taken in that zone: KRX opens
    at 09:00 KST, which is still the *previous* calendar day in US Eastern,
    so using the wrong zone picks the close from two sessions back."""
    current = None
    ...
    session_date = _market_date(quote.get("timestamp"), market_tz)
