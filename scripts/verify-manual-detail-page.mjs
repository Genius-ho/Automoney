...
const root=new URL('../',import.meta.url).pathname.replace(/^\/(.:)/,'$1').replaceAll('/','\\'),port=3013,base=`http://127.0.0.1:${port}`;let child,browser,db;
...
try{
  ...
  const fixtureDir=process.env.DETAIL_PAGE_FIXTURE_DIR||`${process.env.TEMP||process.env.TMP}\\automoney-detail-page-fixtures`;
  ...
  for(let i=1;i<=10;i+=1)await writeFile(`${fixtureDir}\\img${i}.png`,png);
  const shuffledFiles=[6,3,10,1,8,2,9,4,7,5].map((n)=>`${fixtureDir}\\img${n}.png`);
  ...
  await page.screenshot({path:`${root}\\artifacts\\manual-detail-page-draft-64.png`,fullPage:true});
  await writeFile(`${root}\\artifacts\\manual-detail-page-draft-64-result.json`,JSON.stringify(result,null,2));
