#!/usr/bin/env node
import { loadCoupangConfig, loadDatabaseUrl, loadEnvConfig } from '../src/config.mjs';
...
// alert on a product with a live Coupang listing now also suspends that
// listing's items automatically (runSupplierMonitorAndSuspendSweep).
...
const results = await runSupplierMonitorAndSuspendSweep(db, domemeClient, coupangClient);
