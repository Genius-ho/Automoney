#!/usr/bin/env node
import { loadCoupangConfig, loadEnvConfig, loadDatabaseUrl } from '../src/config.mjs';
...
// that writes to Coupang (invoice upload), so every shipment it touches has
// already passed 14.3's carrier-code mapping and a fresh cancellation
// re-check inside dispatchSupplierOrderToChannel.
...
const results = await runChannelDispatchSweep(db, { coupangClient });
let sent = 0;
