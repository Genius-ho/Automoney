...
<body>
  ...
  <script>
    ...
    renderAiPromptSectionsV1=async function(id){await renderAiPromptSectionsWithDetailOrderV1(id);const files=document.querySelector('[data-manual-detail-workflow] [name="images[]"]'),order=document.querySelector('[data-manual-detail-workflow] [data-detail-file-order]');if(!files||!order)return;let selected=[];const renderOrder=()=>{selected=[...files.files];order.innerHTML=selected.map((file,index)=>'<li draggable="true" data-detail-order="'+index+'">'+(index+1)+'. '+escapeHtml(file.name)+'</li>').join('');order.querySelectorAll('[data-detail-order]').forEach(item=>{item.ondragstart=e=>e.dataTransfer.setData('text/plain',item.dataset.detailOrder);item.ondragover=e=>e.preventDefault();item.ondrop=e=>{e.preventDefault();const from=Number(e.dataTransfer.getData('text/plain')),to=Number(item.dataset.detailOrder);if(!Number.isInteger(from)||!Number.isInteger(to)||from===to)return;const reordered=[...selected],moved=reordered.splice(from,1)[0];reordered.splice(to,0,moved);const transfer=new DataTransfer();reordered.forEach(file=>transfer.items.add(file));files.files=transfer.files;renderOrder();};});};files.onchange=renderOrder;};
  </script>
</body>
</html>`;
}
