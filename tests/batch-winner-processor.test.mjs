...
import { processWinnerCandidate } from '../src/batch-winner-processor.mjs';
...
test('processWinnerCandidate skips (never creates a draft) when a draft with the same supplier_product_no already exists', async () => {
  const deps = baseDeps({ findDraftBySupplierProductNoImpl: async () => 27 });
  let createCalled = false;
  const outcome = await processWinnerCandidate({}, candidateRow(), {
    ...deps,
...
test('processWinnerCandidate runs the full pipeline end to end and lands on awaiting_image_approval', async () => {
  const deps = baseDeps();
  const outcome = await processWinnerCandidate({}, candidateRow(), deps);

  assert.equal(outcome.outcome, 'success');
  assert.equal(outcome.draftId, 501);
  const statuses = deps.statusUpdates.map((u) => u.processingStatus).filter(Boolean);
  assert.deepEqual(statuses, ['draft_created', 'analysis_running', 'analysis_completed', 'image_generation_running', 'awaiting_image_approval']);
...
test('processWinnerCandidate never requests manufacturer/countryOfOrigin/handlingPrecautions for auto-apply', async () => {
  ...
  await processWinnerCandidate({}, candidateRow(), deps);
